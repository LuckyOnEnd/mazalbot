...
from ..core.config import INSTRUCTIONS_FILE_PATH, CORRECT_FILE_EXAMPLE_PATH
...
async def handle_error_message(user_id: int, message: types.Message, error_message: str):
    ...
    if unsuccessful_attempts[user_id] >= 3:
...
async def send_instructions_file(user_id: int, message: types.Message):
    ...
    with open(CORRECT_FILE_EXAMPLE_PATH, "rb") as file:
        correct_file_content = file.read()

    media_group = MediaGroupBuilder(caption=_("instructions_file_description"))
    media_group.add_document(BufferedInputFile(file_content, filename="Instructions.pdf"))
    media_group.add_document(BufferedInputFile(correct_file_content, filename="correct_example.csv"))
