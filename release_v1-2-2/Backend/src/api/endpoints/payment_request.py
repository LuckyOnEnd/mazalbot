...
async def payment_request(request: Request):
    ...
    if not is_successful_payment_request(request_json):
        logger.warning("Payment request was not successful. Request json: {}", request_json)
        return Response(status_code=200, content="Payment request was not successful")
